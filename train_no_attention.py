...
def train(config, train_data, encoder, decoder):
    ...
    # pdb.set_trace()
    torch.save(decoder.state_dict(), os.path.join(config.model_dir, Model_Decoder_NA_Dir))
    torch.save(encoder.state_dict(), os.path.join(config.model_dir, Model_Encoder_NA_Dir))
